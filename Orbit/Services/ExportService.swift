...
enum ExportService {
    ...
    static func export(
        ...
        let payload: [String: Any] = [
            ...
            "ideas": ideas.map { ["id": $0.id.uuidString, "title": $0.title, "content": $0.content, "tags": $0.tags, "pinned": $0.pinned, "parentId": json($0.parentID?.uuidString), "folderId": json($0.folderID?.uuidString), "workspaceId": json($0.workspaceID?.uuidString), "canvasX": json($0.canvasX), "canvasY": json($0.canvasY), "createdAt": iso.string(from: $0.createdAt), "updatedAt": iso.string(from: $0.updatedAt)] },
            "workspaces": workspaces.map { ["id": $0.id.uuidString, "name": $0.name, "icon": $0.icon, "orderIdx": $0.orderIndex, "createdAt": iso.string(from: $0.createdAt)] },
            "ideaFolders": ideaFolders.map { ["id": $0.id.uuidString, "name": $0.name, "workspaceId": json($0.workspaceID?.uuidString), "createdAt": iso.string(from: $0.createdAt)] },
            "ideaLinks": ideaLinks.map { ["id": $0.id.uuidString, "ideaAId": $0.sourceID.uuidString, "ideaBId": $0.targetID.uuidString, "relationship": "uses", "createdAt": iso.string(from: $0.createdAt)] },
            "tasks": tasks.map { ["id": $0.id.uuidString, "title": $0.title, "note": $0.note, "done": $0.done, "canvasX": json($0.canvasX), "canvasY": json($0.canvasY), "createdAt": iso.string(from: $0.createdAt), "completedAt": json($0.completedAt.map(iso.string)), "dueDate": json($0.dueDate.map(iso.string))] },
