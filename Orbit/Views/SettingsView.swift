...
struct SettingsView: View {
    ...
    private func exportData() { do { try ExportService.export(habits: habits, habitLogs: logs, ideas: ideas, ideaLinks: ideaLinks, ideaFolders: ideaFolders, tasks: tasks, taskSteps: steps, stepLinks: stepLinks, boardStrokes: boardStrokes, boardNotes: boardNotes, contacts: contacts, interactions: interactions, settings: settings, workspaces: workspaces); exportMessage = "Export finished." } catch { exportMessage = "Could not export: \(error.localizedDescription)" } }
    ...
    private func wipeAllData() {
        logs.forEach(modelContext.delete); habits.forEach(modelContext.delete); ideaLinks.forEach(modelContext.delete); ideas.forEach(modelContext.delete); ideaFolders.forEach(modelContext.delete); stepLinks.forEach(modelContext.delete); boardStrokes.forEach(modelContext.delete); boardNotes.forEach(modelContext.delete); steps.forEach(modelContext.delete); tasks.forEach(modelContext.delete); interactions.forEach(modelContext.delete); contacts.forEach(modelContext.delete); workspaces.forEach(modelContext.delete); try? modelContext.save()
